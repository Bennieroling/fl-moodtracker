...
1. What Changed — one screen, reuses Analytics primitives
1. Readiness Score — most visible, largest scope, best as a keystone
...
1. Determine direction (`high` if value > mean, else `low`)
1. Determine kind using the metric’s `goodDirection` from the catalog:
...
1. Upsert into `anomalies` via the unique constraint
...
1. **Dashboard badge** — small tile or corner indicator: “3 anomalies
...
> 1. **Detection function**. Write `detect_anomalies()` as PL/pgSQL. For
...
> 1. **Cron**. Schedule the function nightly at 03:00 UTC via pg_cron,
>    matching the pattern used for `sync_hae_to_production`.
> 1. **Manual test**. Call the function manually, then `SELECT * FROM anomalies ORDER BY z_score DESC LIMIT 10`. Report the output to
>    the user to sanity-check that flagged readings are in fact
>    unusual.
> 1. **Anomalies page**. Add `/analytics/anomalies` as a 4th tab.
...
> 1. **Dashboard badge**. Add a small indicator to the Dashboard that
...
> 1. **Static page**. Create `src/app/analytics/reports/what-changed/page.tsx`.
...
> 1. **Narrative generation**. Extend the existing Insights API route
...
> 1. **Cache**. Store the generated narrative keyed by `(user_id, week_start_date)` so regenerating the page for the same week
>    doesn’t call the LLM again. Use the existing `insights` table if
>    it has space; otherwise add a minimal `narrative_cache` table.
> 1. **Wire it up**. Page fetches cache first, falls back to LLM call,
...
1. **HRV (30%)** — compare to user’s 60-day rolling mean. Score 50 =
   at mean. Score 100 = +1σ or better. Score 0 = -2σ or worse. Linear
   in between.
1. **Resting HR (20%)** — same approach as HRV, inverted (lower is
   better)
1. **Training load (15%)** — 3-day acute load vs 14-day chronic load
...
> 1. **Scoring function**. Write `compute_readiness(p_user_id uuid, p_date date)` in PL/pgSQL. Implements the four sub-scores per the
...
> 1. **Detail page**. Route `/analytics/readiness`. Same hero + 90-day
>    trend chart + “How it’s computed” section. Make the Dashboard
>    ring tappable and link here.
> 1. **Empty state**. If fewer than 14 days of HRV and RHR data
...
1. Anomaly Detection — smallest new surface area, no UI frontpage
   changes
1. What Changed — reuses a lot of Analytics primitives
1. Readiness Score — largest and most visible, worth doing last once
