...
export const MoodEntryInsertSchema = MoodEntrySchema.omit({
  id: true,
  created_at: true,
  updated_at: true,
})
...
export const FoodEntryInsertSchema = FoodEntrySchema.omit({
  id: true,
  created_at: true,
  updated_at: true,
})
...
export const UserPreferencesSchema = z.object({
  ...
  reminder_time: z
    .string()
    .regex(/^\d{2}:\d{2}:\d{2}$/)
    .default('09:00:00'), // HH:MM:SS format
...
export const UserPreferencesUpdateSchema = UserPreferencesSchema.partial().required({
  user_id: true,
})
